...
`src/lib/listing-sort.ts` — 목록 정렬 **키와 라벨**(매칭점수·이동효율·가격).
어떻게 줄 세우는지는 여기 없다 — **페이지를 나눠 주는 쪽이 정렬도 한다.** 받은
페이지만 화면에서 다시 줄 세우면 다음 페이지가 붙을 때 그 사이에 끼어들기 때문이다.
지금 그 역할은 목(`mocks/listings.ts`)이 대신하고, 가격 축을 환산보증금(보증금 +
월세×100)으로 흉내 내는 임시 규칙도 거기 있다 — 목록 API 가 붙으면 목과 함께 사라지고
이 키는 그대로 서버 파라미터가 된다.

`src/lib/listing-paging.ts` — 목록 화면의 무한 스크롤(페이지 상태·정렬 변경 시 재조회).
`src/lib/api/listings-page.ts` — 한 페이지의 계약(`ListingQuery` / `ListingPage`).
**지도는 페이지를 쓰지 않는다** — 핀과 클러스터에는 보이는 영역의 매물이 전부 있어야
해서, 지도용 조회와 목록용 페이지 조회가 갈라져 있다.
