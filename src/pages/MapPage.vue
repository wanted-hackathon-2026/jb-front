...
const TABS = [
  // AI 추천이 이 서비스가 파는 것이라 먼저 읽히는 자리에 두고, 첫 화면에서 골라져
  // 있는 것도 이쪽이다(stores/sheet.ts 의 DEFAULT_SHEET_TAB). 주변 매물은 시트 뒤
  // 지도에 이미 보이므로, 시트를 올렸을 때 할 수 있는 일이 먼저 나오는 게 맞다.
  ...
  <main class="relative flex-1 overflow-hidden">
    ...
    >
      <div class="flex flex-col items-end gap-3">
        ...
        >
          <!--
            시안 export 를 제 크기(20×19)로 그린다. size-6 같은 정사각 클래스로 부르면
            세로가 긴 쪽에 맞춰 늘어나고(20×19 는 24×22.8, 아래 하트는 24×21.2),
            두 아이콘이 서로 다른 배율로 커져 나란히 뜬 FAB 의 무게가 어긋난다.
          -->
          <svg viewBox="0 0 20 19" class="h-[19px] w-5" fill="none" aria-hidden="true">
            <path
        ...
        <button
          ...
          @click="openFavorites"
        >
          <svg viewBox="0 0 17 15" class="h-[15px] w-[17px]" fill="none" aria-hidden="true">
            <path
