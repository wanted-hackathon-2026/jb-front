...
/**
 * 서버가 주는 시각. **그대로 `new Date()` 에 넣으면 안 된다.**
 *
 * 오프셋 없이 내려와서(`2026-09-20T13:12:52.942674`) 브라우저가 로컬 시각으로 읽는다 —
 * KST 에서 9시간 어긋난다. `lib/server-time.ts` 의 `toInstant()` 로 세운 뒤 쓴다.
 * 잊어도 괜찮게 해뒀다: 시각을 받는 자리가 `Instant` 를 요구해서 타입 검사가 막는다.
 */
