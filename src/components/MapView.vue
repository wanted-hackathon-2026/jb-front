...
.jb-map :deep(svg:not(.jb-anchor-label) :is(ellipse, circle, path)) {
  ...
  */
  ...
  /*
    그라데이션 전체의 진하기. **중심을 진하게 하는 레버는 여기 하나뿐이다** — 중심
    stop 은 이미 1 이고, 색을 brand-600 으로 내리는 길은 막혀 있다(main.css: 화면에
    보이는 민트는 500 하나).

    0.55 는 지도 글자가 원 안에서 읽히는 선이다. 더 올리면 중심부의 지명·역 이름이
    민트에 잠긴다 — 도달권은 지도를 가리자고 있는 게 아니라 어디까지인지 말하자고 있다.
  */
...
/*
  ...
  세 단계인 건 가운데에서 한 번 머물다 떨어지게 하려는 것이다. 두 단계로 곧게 이으면
  중심의 진한 덩어리가 사라져 원이 통째로 고르게 뿌예진다.

  ⚠️ 감쇠는 **시안보다 가파르다.** 시안 캡처의 중심 거리별 값(R 기준 107 → 88 → 80 →
  62 → 43 → 27)을 그대로 옮기면 가장자리가 0.25 로 남는데, 전역 fill-opacity 0.4 가
  곱해지고 나면 중심 0.40 대 가장자리 0.10 이라 화면에서는 원 전체가 거의 같은 농도로
  보였다 — 채움이 어디서 끝나는지 눈으로 알 수 없었다.

  그래서 바깥을 0 까지 뺀다. 도달권은 가장자리로 갈수록 덜 확실한 영역이라 색도 거기서
  사라지는 게 맞고, 경계가 어디냐는 파선이 대신 말한다.
*/
...
.jb-map :deep(.jb-ring-mid) {
  stop-color: var(--color-brand-500);
  /* 위 fill-opacity 를 올린 만큼 내린다 — 중간 지점의 실제 농도는 그대로 두고
     중심만 진해지게. 0.5 × 0.55 = 0.275 로 종전(0.6 × 0.4 = 0.24)과 비슷하다. */
  stop-opacity: 0.5;
}
